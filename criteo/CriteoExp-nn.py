...
if not os.path.exists('criteo-10-nn.pt'):
    best = float('inf')
    for i in range(10):
        model.train()
        ...
        if logloss < best:
            print('Improved!')
            best = logloss
            torch.save(model.state_dict(), 'criteo-10-nn-best.pt')
    ...
    torch.save(model.state_dict(), 'criteo-10-nn.pt')
else:
    model.load_state_dict(torch.load('criteo-10-nn.pt'))
