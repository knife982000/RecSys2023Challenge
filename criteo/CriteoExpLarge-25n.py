...
model = DeepMF(ds_train, dims=64).to(device)
...
if not os.path.exists('criteo-40l-25n.pt'):
    best = float('inf')
    for i in range(40):
        model.train()
        ...
        if logloss < best:
            print('Improved!')
            best = logloss
            torch.save(model.state_dict(), 'criteo-40l-25n-best.pt')
    ...
    torch.save(model.state_dict(), 'criteo-40l-25n.pt')
else:
    model.load_state_dict(torch.load('criteo-40l-25n.pt'))
