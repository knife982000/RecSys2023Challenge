...
if not os.path.exists('criteo-40L.pt'):
    best = float('inf')
    for i in range(40):
        model.train()
        ...
        if logloss < best:
            print('Improved!')
            best = logloss
            torch.save(model.state_dict(), 'criteo-40L-best.pt')
    ...
    torch.save(model.state_dict(), 'criteo-40L.pt')
else:
    model.load_state_dict(torch.load('criteo-40.pt'))
