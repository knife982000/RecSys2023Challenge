...
if not os.path.exists('criteo-10l-nn.pt'):
    best = float('inf')
    for i in range(10):
        model.train()
        ...
        if logloss < best:
            print('Improved!')
            best = logloss
            torch.save(model.state_dict(), 'criteo-10l-nn-best.pt')
    ...
    torch.save(model.state_dict(), 'criteo-10l-nn.pt')
else:
    model.load_state_dict(torch.load('criteo-10l-nn.pt'))
